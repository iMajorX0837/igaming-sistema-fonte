...
export default function HomeSectionsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  const loadHomeBackground = async () => {
    try {
      ...
      if (error) {
        showToast('Erro ao carregar cor da home. Execute Deploy-Infra/supabase_nova_casa.sql no Supabase.', 'error');
        return;
      }
  ...
  const loadSections = async () => {
    try {
      ...
      if (error) {
        showToast('Execute Deploy-Infra/supabase_nova_casa.sql no Supabase.', 'error');
        return;
      }
    ...
    try {
      ...
      if (error) {
        if (error.code === '23505') {
          ...
          showToast('Erro ao criar seção. Execute Deploy-Infra/supabase_nova_casa.sql no Supabase.', 'error');
        }
        return;
      }
