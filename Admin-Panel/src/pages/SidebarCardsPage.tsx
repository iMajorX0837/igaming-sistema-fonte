...
export default function SidebarCardsPage() {
  ...
  const loadSiteColors = async () => {
    try {
      ...
      if (error) {
        showToast('Erro ao carregar cores do site. Execute Deploy-Infra/supabase_nova_casa.sql no Supabase.', 'error');
        return;
      }
  ...
  const saveAuthModalsImages = async () => {
    ...
    try {
      ...
      if (upsertError) {
        showToast('Erro ao salvar imagens dos modais. Execute Deploy-Infra/supabase_nova_casa.sql no Supabase.', 'error');
        return;
      }
  ...
  const saveBrandColors = async () => {
    ...
    try {
      ...
      if (upsertError) {
        showToast('Erro ao salvar cores de destaque. Execute Deploy-Infra/supabase_nova_casa.sql no Supabase.', 'error');
        return;
      }
