...
export function usePaymentGatewayConfig() {
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      if (gatewayRes.error) {
        showToast(
          'Erro ao carregar gateways. Execute Deploy-Infra/supabase_nova_casa.sql no Supabase.',
          'error',
        );
        return;
      }
      ...
      if (misticpayRes.error) {
        showToast(
          'Erro ao carregar MisticPay. Execute Deploy-Infra/supabase_nova_casa.sql no Supabase.',
          'error',
        );
      } else {
