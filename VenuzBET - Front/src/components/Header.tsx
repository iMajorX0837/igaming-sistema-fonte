...
import { getHeaderFooterCopy, getLocaleForLanguage } from '../i18n/headerFooter';
              ...
              <>
                ...
                <div className="relative account-menu-container">
                  <button 
                    ...
                    aria-label={`${copy.accountMenuAria} — ${user.name}`}
                  ...
                  <div className={`absolute right-0 top-full mt-2 w-56 border border-white/10 rounded-lg shadow-xl transition-all duration-200 z-[100] ${isAccountMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`} style={{ backgroundColor: headerConfig.fundo }}>
                    ...
                    {proximoNome ? (
                      <div className="px-4 pb-3 flex items-center gap-1.5 w-full">
                        <span className="text-xs text-white font-bold shrink-0">{copy.nextVip}</span>
                        {proximoImage && (
                          <img
                            src={proximoImage}
                            alt={proximoNome}
                      ...
                      <div className="px-4 pb-3">
                        <p className="text-xs text-white font-bold">{copy.maxVipLevel}</p>
                      </div>
                    )}
                    <div className="py-1">
              ...
              <div className="flex items-center gap-2 md:gap-3 ml-auto">
                ...
                >
                  ...
                  <span className="relative z-10">
                    <span className="hidden md:inline">{copy.register}</span>
                    <span className="md:hidden">{copy.registerShort}</span>
                  </span>
                </button>
              </div>
            )}
