#!/usr/bin/env bash
# Copia API-BackEnd/.env (do git pull) para Deploy-Infra/tenants/<tenant>/env.api
...
API_LOCAL="$REPO_ROOT/API-BackEnd/.env"
...
if [[ ! -f "$API_LOCAL" ]]; then
  echo "Erro: $API_LOCAL não encontrado (git pull traz API-BackEnd/.env)."
  exit 1
fi
