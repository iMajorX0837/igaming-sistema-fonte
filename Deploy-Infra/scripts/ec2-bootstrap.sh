...
# curl -fsSL ... | bash   OU   bash Deploy-Infra/scripts/ec2-bootstrap.sh
...
if [[ -d /opt/venuzbet/Deploy-Infra ]]; then
  chmod +x /opt/venuzbet/Deploy-Infra/scripts/*.sh
  echo "==> OK. Próximo:"
  echo "    cd /opt/venuzbet/Deploy-Infra"
  ...
  echo "==> Pasta Deploy-Infra não encontrada. Faça git clone primeiro."
fi
