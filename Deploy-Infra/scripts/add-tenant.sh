...
Próximos passos:
1. Edite Deploy-Infra/tenants/$SLUG/env.api (Supabase, MisticPay, etc.)
2. Aponte DNS:
   - $DOMAIN
   - admin.$DOMAIN
   - api.$DOMAIN
3. bash Deploy-Infra/scripts/build-tenant.sh $SLUG
4. bash Deploy-Infra/scripts/up-tenant.sh $SLUG
