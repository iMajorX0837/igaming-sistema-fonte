...
export function createSupabaseProxyClient(options: SupabaseProxyClientOptions = {}) {
  ...
  const auth = {
    ...
    async verify2FALogin(params: { challengeToken: string; code: string }) {
      ...
      if (payload.error) {
        return {
          data: {
            user: null,
            session: null,
            challengeToken: payload.data?.challengeToken ?? params.challengeToken,
          },
          error: payload.error,
        };
      }
    ...
    twoFactor: {
      async getStatus() {
        ...
        const payload = await response.json();
        return { data: payload.data ?? { enabled: false }, error: payload.error ?? null };
      },

      async setup() {
        const response = await apiFetch('/auth/2fa/setup', { method: 'POST', body: '{}' });
        const payload = await response.json();
